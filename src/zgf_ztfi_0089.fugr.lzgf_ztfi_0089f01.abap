...
FORM fo_new_entry.
  ...
  PERFORM fo_authority_check USING    ztfi_0089-bukrs
                             CHANGING sy-subrc.
  IF sy-subrc NE 0.
    MESSAGE e800(fr) WITH ztfi_0089-bukrs.
  ENDIF.
...
**//.. Filtrar datos por autorizarión
  DATA: lt_ztfi_0089 TYPE TABLE OF ztfi_0089,
        ls_ztfi_0089 TYPE ztfi_0089.

  LOOP AT <primtab> INTO <vim_total_struc>.
    ls_ztfi_0089 = <vim_total_struc>.

    PERFORM fo_authority_check USING    ls_ztfi_0089-bukrs
                               CHANGING sy-subrc.
    IF sy-subrc NE 0.
      DELETE <primtab>.
    ENDIF.
