...
FORM fo_get_data.
  ...
  DATA: lt_extract      TYPE TABLE OF ty_extract,
        ls_data_total   TYPE ty_extract,
        ls_data_extract TYPE ty_extract,
        lv_action_total TYPE comt_boolean,
        ls_tabix        TYPE sy-tabix.

  FIELD-SYMBOLS: <fs_action_total>    TYPE char1,
                 <fs_data_total>      TYPE ANY,
                 <fs_data_extract>    TYPE ANY.

  IF sy-uname EQ 'NO_OTOCHON'.
    BREAK-POINT.
...
**//.. Filtrar datos por autorizarión
  DATA: lt_ztfi_0076 TYPE TABLE OF ztfi_0076,
        ls_ztfi_0076 TYPE ztfi_0076.

  LOOP AT <primtab> INTO <vim_total_struc>.
    ls_ztfi_0076 = <vim_total_struc>.

    PERFORM fo_authority_check USING    ls_ztfi_0076-bukrs
                               CHANGING sy-subrc.
    IF sy-subrc NE 0.
      DELETE <primtab>.
    ENDIF.
...
*----------------------------------------------------------------------*
FORM fo_new_entry.
  DATA: ls_extract      TYPE ty_extract.

*  ls_extract = extract.

  PERFORM fo_authority_check USING    ztfi_0076-bukrs
                             CHANGING sy-subrc.
  IF sy-subrc NE 0.
    MESSAGE e800(fr) WITH ztfi_0076-bukrs.
  ENDIF.
